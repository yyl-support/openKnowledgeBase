...
本次数据快照日期为 **2026-08-04**。组织仓库统计来源为公开仓库中的[聚合快照](org-repos-snapshot.json)，不包含仓库清单。服务注册项的逐项范围见 [`source-registry.md`](source-registry.md)，分类与服务入口见 [`service-map.md`](service-map.md)。
...
原始授权 GitHub API 返回的仓库级数据未复制到本公开仓库；公开读者可复核聚合结果，但不能从本仓恢复仓库清单。以上来源无法核验的部分不补造；受控 backlog 的内容也不因出现在来源清单中而被描述为公开可访问。
