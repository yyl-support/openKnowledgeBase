---
tags: [ci, ci-all]
service: ci-all
source: https://github.com/opensourceways/ci-all
updated_at: 2026-08-04
---

## summary
Jenkins / CodeArts CI通用工具链，注册说明包含脚本同步和日志扫描用途。代码仓库是cascading umbrella，具体子仓以仓库目录树为准。

## infrastructure
- 服务注册文件将实现仓库指定为opensourceways/ci-all，引用main分支。
- 注册文件描述其用途为Jenkins / CodeArts CI通用工具链（脚本同步 / 日志扫描）。

## components

| name | role |
| --- | --- |
| ascend_optimization_scripts | 仓库目录项，未取得子仓类型或运行方式。 |
| codearts-ci-config | 仓库目录项，未取得子仓类型或运行方式。 |
| codearts-workflow-image | 仓库目录项，未取得子仓类型或运行方式。 |
| gitcode-migrate-script | 仓库目录项，未取得子仓类型或运行方式。 |
| jenkins-log-scanner | 仓库目录项，未取得子仓类型或运行方式。 |
| sync-file-server | 仓库目录项，未取得子仓类型或运行方式。 |

## 源码

- source_repo: https://github.com/opensourceways/ci-all
- sub_repos:
  - ascend_optimization_scripts
  - codearts-ci-config
  - codearts-workflow-image
  - gitcode-migrate-script
  - jenkins-log-scanner
  - sync-file-server

导航：[`README`](../../../../README.md) · [`org-overview`](../../../org-overview.md) · [`service-map`](../../../service-map.md) · [`source-registry`](../../../source-registry.md)
